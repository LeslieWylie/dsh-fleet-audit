...
import { findSecretProviders, stripEmbeddedCredentials } from './secret.ts'
import type { AuditArgs, AuditResult, FileCheck, GitLeak, SecretHit } from './types.ts'
...
async function checkFile(p: string): Promise<FileCheck> {
  ...
  if (mode === null) return { path: p, exists: false, mode: '---', tooOpen: false }
  const n = parseInt(mode, 8)
  return { path: p, exists: true, mode, tooOpen: (n & 0o077) !== 0 }
}

async function collectGitConfigs(roots: string[], max: number, maxDepth: number): Promise<string[]> {
  const found = new Set<string>()
  if (max > 0) found.add(path.join(os.homedir(), '.gitconfig'))
  const stack: { dir: string; depth: number }[] = roots.map(r => ({ dir: r, depth: 0 }))
...
function extractGitLeaks(file: string, text: string): GitLeak[] {
  const leaks: GitLeak[] = []
...
export async function runAudit(args: unknown): Promise<AuditResult> {
  if (typeof args !== 'object' || args === null) throw new Error('fleet_audit: args must be an object')
  const a = args as Record<string, unknown>
  for (const k of Object.keys(a)) {
    if (!KNOWN_ARG_KEYS.has(k)) throw new Error(`fleet_audit: unknown option "${k}"`)
  }
  const roots = Array.isArray(a.roots) ? (a.roots as string[]) : []
  const extraFiles = Array.isArray(a.files) ? (a.files as string[]) : []
  ...
  for (const f of files) fileChecks.push(await checkFile(f))
  for (const d of DEFAULT_DIRS) fileChecks.push(await checkFile(resolveHome(d)))

  const configs = await collectGitConfigs(roots, maxGit, maxDepth)
  const gitLeaks: GitLeak[] = []
  ...
  for (const cfg of configs) {
    ...
    if (scanSecrets) {
      ...
      if (provs.length > 0) secretHits.push({ file: cfg, providers: provs })
    }
  }
